...
/* ═══════════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════════ */
const KanbanBoard = ({tasks, onSelect, onUpdate, onToggle, flash}) => {
      ...
      {overdueTasks.length > 0 && (
        <div style={{minWidth:220,maxWidth:280,flex:"1 0 220px",display:"flex",flexDirection:"column",background:"#fef2f2",borderRadius:14,border:"1px solid #fecaca",overflow:"hidden"}}>
          ...
          <div style={{flex:1,overflowY:"auto",padding:8}}>
            {overdueTasks.map(t => (
              <KanbanCard key={t.id} task={t} onSelect={onSelect} onToggle={onToggle} />
            ))}
          </div>
        </div>
      )}
...
const KanbanCard = ({task, onSelect, onToggle}) => {
  ...
  return (
    <div draggable
      ...
      <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
        <div style={{paddingTop:1}} onClick={e => { e.stopPropagation(); onToggle(task.id); }}>
          <Checkbox checked={task.completed} priority={task.priority} size={16}/>
        </div>
        <div style={{flex:1,minWidth:0}}>
      ...
      <nav className="sidebar" aria-label="Navigation" style={{width:sidebar?264:0,...(isMobile?{position:"fixed",zIndex:100,height:"100vh"}:{}),background:"var(--surface)",borderRight:"1px solid var(--border)",display:"flex",flexDirection:"column",transition:"width 0.25s ease",overflow:"hidden",flexShrink:0}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"0 8px"}}>
          <NavSection title="Views">
            {(()=>{
              const tomorrowCount=tasks.filter(t=>!t.completed&&t.dueDate===tomorrowStr()).length;
              const nextMonCount=tasks.filter(t=>!t.completed&&t.dueDate===nextMondayStr()).length;
              const dateDrop=(dateStr,label)=>({
                ...
                onDrop:e=>{e.preventDefault();e.currentTarget.style.background="";e.currentTarget.style.outline="";const tid=e.dataTransfer.getData("text/plain");if(!tid)return;const src=e.dataTransfer.getData("application/x-source");
                  if(src==="subtask"){setTasks(prev=>{let sub=null;for(const t of prev){sub=findSubById(t.subtasks,tid);if(sub)break;}if(!sub)return prev;const cleaned=prev.map(t=>({...t,subtasks:removeSubById(t.subtasks||[],tid)}));const promoted={id:sub.id,title:sub.title,completed:sub.completed||false,dueDate:dateStr,startDate:sub.startDate||null,endDate:sub.endDate||null,priority:sub.priority||"none",list:"Inbox",subtasks:sub.subtasks||[],notes:sub.notes||"",tags:sub.tags||[],createdAt:new Date().toISOString(),completedAt:null};return[promoted,...cleaned];});flash(`Promoted to ${label}`);}
                  else{const ids=selectedIds.size>1&&selectedIds.has(tid)?selectedIds:new Set([tid]);setTasks(prev=>prev.map(t=>ids.has(t.id)?{...t,dueDate:dateStr}:t));flash(`Set ${ids.size>1?ids.size+" tasks":"task"} to ${label}`);setSelectedIds(new Set());}
                }
              });
              const views=[
                {id:"today",icon:Icons.today,label:"Today",count:todayCount,drop:dateDrop(todayStr(),"Today"),dateStr:todayStr()},
                {id:"tomorrow",icon:Icons.upcoming,label:"Tomorrow",count:tomorrowCount,drop:dateDrop(tomorrowStr(),"Tomorrow"),noView:true,dateStr:tomorrowStr()},
                {id:"nextmon",icon:Icons.calendar,label:"Next Monday",count:nextMonCount,drop:dateDrop(nextMondayStr(),"Next Monday"),noView:true,dateStr:nextMondayStr()},
              ...
              return views.map(({id,icon,label,count,drop,noView,dateStr:ds})=>(
                <div key={id} {...(drop||{})} data-drop-type={ds?"date":undefined} data-drop-value={ds||undefined}>
                  <NavItem active={view===id} icon={icon} label={label} count={showViewCounts?count:0}
                    onClick={noView?undefined:()=>selectView(id)}
                    countColor={id==="today"&&overdueCount>0?"#ef4444":undefined}/>
                </div>
              ));
            })()}
          </NavSection>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
        ...
        <div style={{flex:1,display:"flex",overflow:"hidden"}}>
          <div style={{flex:1,overflowY:"auto",padding:isMobile?"16px":"20px 24px"}}>
            {view==="calendar"?(<CalendarView tasks={tasks} onSelect={t=>setSelectedTask(t)} onUpdate={updateTask}/>):view==="today"?(<div style={{display:"flex",flexDirection:"column",height:"100%"}}>
              ...
              <div style={{flex:1,minHeight:0}}><KanbanBoard tasks={tasks} onSelect={t=>setSelectedTask(t)} onUpdate={updateTask} onToggle={toggleTask} flash={flash}/></div>
            </div>):(<>
