...
const Overlay = ({onClose,children,wide}) => {
  ...
  <div style={{position:"fixed",inset:0,background:"var(--overlay)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,animation:"fadeIn 0.2s ease",padding:16}} onClick={onClose} role="dialog" aria-modal="true">
    <div className="glass-deep" onClick={e=>e.stopPropagation()} style={{background:"var(--bg)",borderRadius:"var(--radius-xl)",padding:28,width:"100%",maxWidth:wide?560:500,boxShadow:"var(--shadow-3)",position:"relative",display:"flex",flexDirection:"column",maxHeight:"90vh"}}>
...
const SnoozeButton = ({task, onSnooze}) => {
  ...
  return (<>
    ...
    {menu&&(<>
      ...
      <div role="menu" className="glass" onClick={e=>e.stopPropagation()}
...
const LoginScreen = ({ onSignIn, onSignInPassword, error }) => {
  ...
  return (
    <div style={{minHeight:"100dvh",display:"flex",alignItems:"center",justifyContent:"center",background:"var(--bg)",fontFamily:"var(--font-body)",padding:20}}>
      <style>{`
        :root{--font-display:'Fraunces','Playfair Display',Georgia,serif;--font-body:'Inter Tight','Source Sans 3',-apple-system,sans-serif;--font-mono:'JetBrains Mono',ui-monospace,Consolas,monospace;--ink:#222941;--text:#464b63;--muted:#7b7968;--bg:#faf7f0;--surface:#f2ece0;--border:#e4ddd0;--accent:#a8642e;--accent-dark:#8a5426;--accent-bg:#fdf4e3;--accent2:#c98a45;--card:#fffcf5;--danger:#b85454;--shadow:rgba(26,34,56,0.08);--shadow-strong:rgba(26,34,56,0.16);--glass-bg:rgba(255,252,245,0.66);--glass-border:rgba(255,255,255,0.55);--glass-highlight:rgba(255,255,255,0.35);--accent-a15:rgba(168,100,46,0.15);--accent-a20:rgba(168,100,46,0.2);}
...
export default function InkwellApp() {
  ...
  const [kanbanColumns, setKanbanColumns] = useState(() => {
    ...
    /* Auto-expire: remove columns with dates before today, keep future ones */
    const today = todayStr();
    const fresh = saved.filter(c => !c.dateStr || c.dateStr >= today);
  ...
  useEffect(() => {
    const check = () => {
      ...
      setKanbanColumns(prev => {
        ...
        const today = todayStr();
        const fresh = prev.filter(c => !c.dateStr || c.dateStr >= today);
  ...
  return (
    <div className={`${frostMode?(darkMode?"theme-frost-dark":"theme-frost-light"):(darkMode?"theme-dark":"theme-light")}${quillMode?" quill-mode":""}${midnightActive?" midnight":""}${paperUI?" paper-journal":""}`} style={{height:"100dvh",display:"flex",overflow:"hidden",backgroundColor:"var(--bg)",backgroundImage:"var(--paper-noise)",color:"var(--ink)",fontFamily:"var(--font-body)"}}>
      <style>{`
        ...
        <div style={{flex:1,overflowY:"auto",padding:"0 8px"}}>
          ...
          <NavSection title="Lists" action={()=>setShowNewList(true)}>
            ...
            {listMenu&&(<>
              ...
              <div className="glass" style={{position:"fixed",left:listMenu.x,top:listMenu.y,background:"var(--card)",borderRadius:"var(--radius-md)",boxShadow:"var(--shadow-2)",border:"1px solid var(--border)",padding:4,zIndex:1000,minWidth:140,animation:"fadeIn 0.1s ease"}}>
      ...
      <main id="main-content" style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
        ...
        <header className="glass-bar" style={{padding:isMobile?"14px 16px":"14px 24px",borderBottom:"1px solid var(--border-light)",display:"flex",alignItems:"center",gap:12,flexShrink:0,position:"relative",zIndex:10}}>
          ...
          {view!=="calendar"&&(<div style={{position:"relative",flexShrink:0}}>
            ...
            {showSortMenu&&(<>
              ...
              <div className="glass" style={{position:"absolute",top:"100%",right:0,marginTop:6,background:"var(--card)",borderRadius:"var(--radius-md)",border:"1px solid var(--border)",boxShadow:"var(--shadow-2)",padding:8,zIndex:1301,minWidth:200}}>
                  ...
                  {view==="today" && (
                    <div style={{position:"relative"}}>
                      ...
                      {showTodayFilter && (<>
                        ...
                        <div className="glass" style={{position:"absolute",top:"calc(100% + 4px)",left:0,background:"var(--card)",borderRadius:"var(--radius-md)",boxShadow:"var(--shadow-2)",border:"1px solid var(--border)",padding:6,zIndex:1000,minWidth:200,maxHeight:340,overflowY:"auto",animation:"fadeIn 0.12s ease"}}>
      ...
      {/* ═══ BULK ACTION BAR ═══ */}
      {selectedIds.size>1&&(
        <div className="glass" style={{position:"fixed",bottom:24,left:"50%",transform:"translateX(-50%)",color:"var(--ink)",padding:"10px 16px",borderRadius:"var(--radius-lg)",fontSize:13,fontWeight:600,animation:"toastIn 0.3s ease",zIndex:1500,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",justifyContent:"center",maxWidth:"calc(100vw - 24px)"}}>
          <span style={{color:"var(--muted)"}}>{selectedIds.size} selected</span>
          <div style={{width:1,height:20,background:"var(--border)"}}/>
          <select onChange={e=>{if(e.target.value)bulkMove(e.target.value);e.target.value="";}} defaultValue="" style={{background:"var(--surface)",border:"1px solid var(--border)",color:"var(--text)",borderRadius:"var(--radius-sm)",padding:"5px 8px",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>
            <option value="" disabled>Move to…</option>
            {lists.map(l=><option key={l} value={l} style={{color:"var(--ink)"}}>{l}</option>)}
          </select>
          <select onChange={e=>{if(e.target.value)bulkPriority(e.target.value);e.target.value="";}} defaultValue="" style={{background:"var(--surface)",border:"1px solid var(--border)",color:"var(--text)",borderRadius:"var(--radius-sm)",padding:"5px 8px",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>
            <option value="" disabled>Priority…</option>
            {Object.entries(PRIORITY).map(([k,v])=><option key={k} value={k} style={{color:"var(--ink)"}}>{v.label}</option>)}
          </select>
          <button onClick={bulkComplete} style={{background:"var(--surface)",border:"1px solid var(--border)",color:"var(--text)",borderRadius:"var(--radius-sm)",padding:"5px 10px",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>✓ Done</button>
          <input type="date" onChange={e=>{if(e.target.value)bulkDate(e.target.value);}} style={{background:"var(--surface)",border:"1px solid var(--border)",color:"var(--text)",borderRadius:"var(--radius-sm)",padding:"4px 8px",fontSize:12,cursor:"pointer",fontFamily:"inherit"}} title="Set due date"/>
          <button onClick={bulkDelete} style={{background:"var(--danger)",border:"none",color:"white",borderRadius:"var(--radius-sm)",padding:"5px 10px",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>Delete</button>
          <button onClick={()=>setSelectedIds(new Set())} style={{background:"none",border:"none",color:"var(--muted)",cursor:"pointer",padding:4,display:"flex"}}>{Icons.x}</button>
        </div>
      )}
      ...
      {pendingCompleteId&&(()=>{
        ...
        return (
          <div style={{position:"fixed",inset:0,background:"var(--overlay)",zIndex:2500,display:"flex",alignItems:"center",justifyContent:"center",padding:16,animation:"fadeIn 0.15s ease"}}
            ...
            <div className="glass-deep" onClick={e=>e.stopPropagation()} style={{background:"var(--card)",borderRadius:"var(--radius-lg)",padding:"24px 28px",maxWidth:380,width:"100%",boxShadow:"var(--shadow-3)",animation:"toastIn 0.2s ease"}}>
      ...
      {toast&&<div role="status" aria-live="polite" className="glass" style={{position:"fixed",bottom:isDragging?84:24,left:"50%",transform:"translateX(-50%)",color:"var(--ink)",padding:"12px 24px",borderRadius:"var(--radius-lg)",fontSize:14,fontWeight:600,animation:"toastIn 0.3s ease",zIndex:2000,whiteSpace:"nowrap",pointerEvents:"none",transition:"bottom 0.2s ease",fontFamily:"var(--font-body)"}}>{typeof toast==="string"?renderCoffee(toast):toast}</div>}
