    ...
    <div className="detail-panel" role="complementary" aria-label="Task details"
      ...
      <div style={{flex:1,overflowY:"auto",padding:"18px 20px"}}>
        ...
        <div style={{display:"flex",alignItems:"flex-start",gap:12,marginBottom:20}}>
          ...
          <textarea value={title} onChange={e=>setTitle(e.target.value)} rows={1} onBlur={()=>title.trim()&&updateCurrent({title:title.trim()})} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();e.target.blur();}}}
            style={{flex:1,border:"none",background:"none",fontSize:18,fontFamily:"var(--font-display)",fontWeight:700,color:current.completed?"var(--muted)":"var(--ink)",textDecoration:current.completed?"line-through":"none",outline:"none",padding:0,resize:"none",lineHeight:1.3,overflow:"hidden"}}/>
        </div>
