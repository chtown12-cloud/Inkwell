    ...
    <div className="detail-panel" role="complementary" aria-label="Task details"
      ...
      <div style={{flex:1,overflowY:"auto",padding:"18px 20px"}}>
        ...
        <div style={{display:"flex",alignItems:"flex-start",gap:12,marginBottom:20}}>
          ...
          <textarea value={title} onChange={e=>setTitle(e.target.value)} rows={1} onBlur={e=>{if(title.trim())updateCurrent({title:title.trim()});e.target.style.borderColor="transparent";e.target.style.background="none";}} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();e.target.blur();}}}
                    ...
                    {week.map((dStr, col) => {
                      ...
                      return (
                        <div key={col}
                          ...
                          style={{minHeight:56 + barsH,padding:4,background:isH?"rgba(217,119,6,0.12)":isT?"rgba(217,119,6,0.05)":"transparent",borderBottom:"1px solid var(--border-light)",borderRight:col < 6 ? "1px solid var(--border-light)":"none",transition:"background 0.1s"}}>
                          {dayN != null && (
...
const TaskRow = ({task,isActive,onSelect,onToggle,onUpdateTask,onDragStart,onDragOver,onDrop,onDragEnd,view,lists}) => {
  ...
  /* Delayed click: waits 250ms for potential double-click before opening detail */
  const handleRowClick = (e) => {
    if(clickTimer.current) return; // already waiting
    clickTimer.current = setTimeout(()=>{clickTimer.current=null; onSelect(task);}, 250);
  };
  ...
  return (
    <div draggable onDragStart={e=>onDragStart(e,task)} onDragOver={e=>onDragOver(e,task)} onDrop={e=>onDrop(e,task)} onDragEnd={onDragEnd}
      ...
      <div onClick={handleRowClick} style={{display:"flex",alignItems:"flex-start",gap:8,padding:"11px 12px",cursor:"pointer"}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          {/* Double-click editable task title — cancels the row single-click */}
      ...
      {subsOpen&&subTotal>0&&(
        <div style={{paddingLeft:52,paddingBottom:8,paddingRight:12}}>
          <SubtaskTree subtasks={task.subtasks} onAction={handleSubAction}/>
        </div>
      )}
    </div>
  );
...
export default function InkwellApp() {
  ...
  return (
    <div style={{height:"100vh",display:"flex",overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"0 8px"}}>
          <NavSection title="Views">
            {[{id:"today",icon:Icons.today,label:"Today",count:todayCount},{id:"upcoming",icon:Icons.upcoming,label:"Upcoming"},{id:"all",icon:Icons.all,label:"All Tasks",count:tasks.filter(t=>!t.completed).length},{id:"completed",icon:Icons.done,label:"Completed",count:tasks.filter(t=>t.completed).length},{id:"calendar",icon:Icons.calendar,label:"Calendar"}].map(({id,icon,label,count})=>(
              <NavItem key={id} active={view===id} icon={icon} label={label} count={showViewCounts?count:0} onClick={()=>selectView(id)} countColor={id==="today"&&overdueCount>0?"#ef4444":undefined}/>
            ))}
          </NavSection>
          <NavSection title="Lists" action={()=>setShowNewList(true)}>
            {lists.map(l=>{const lv=l==="Inbox"?"inbox":`list:${l}`;const color=getListColor(l,lists);return(
              <div key={l} onDragOver={e=>{e.preventDefault();e.currentTarget.style.background="var(--accent-bg)";e.currentTarget.style.outline="2px solid var(--accent)";e.currentTarget.style.borderRadius="10px";}} onDragLeave={e=>{e.currentTarget.style.background="";e.currentTarget.style.outline="";}} onDrop={e=>{e.currentTarget.style.background="";e.currentTarget.style.outline="";onListDrop(e,l);}}>
      ...
      {showShortcuts&&(<Overlay onClose={()=>setShowShortcuts(false)}><h2 style={{fontSize:19,fontFamily:"var(--font-display)",marginBottom:16}}>Keyboard Shortcuts</h2>{[["N","New task"],["/ or ⌘F","Search"],["Esc","Close panel"],["?","This help"],["Double-click","Edit any name"]].map(([k,d])=>(<div key={k} style={{display:"flex",alignItems:"center",gap:12,padding:"8px 0",borderBottom:"1px solid var(--border-light)"}}><kbd style={{fontSize:12,fontWeight:600,background:"var(--surface)",padding:"3px 8px",borderRadius:6,border:"1px solid var(--border)",fontFamily:"inherit",minWidth:50,textAlign:"center"}}>{k}</kbd><span style={{fontSize:14,color:"var(--text)"}}>{d}</span></div>))}</Overlay>)}
